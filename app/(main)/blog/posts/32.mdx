...
```bash
#!/bin/bash
# written to a file named "update"
...
# use your username, paru and flatpak shouldn't be ran as root
USERNAME=codabool
...
echo -e "\n========== official & AUR ==========\n"
sudo -u $USERNAME paru -Syu --skipreview --removemake --nocleanafter --pgpfetch --noconfirm >> $BACKUP_LOCATION

echo -e "\n====== updating flatpaks ======\n" >> $BACKUP_LOCATION
echo -e "\n========== flatpak ==========\n"
sudo -u $USERNAME /usr/bin/flatpak update --noninteractive --assumeyes >> $BACKUP_LOCATION
