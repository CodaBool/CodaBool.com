...
# since this is intended to run once a week and there are 52 weeks in a year
filename=$(echo "update.$(($(date +%V) % 52)).log")

# WARN: replace with your systems username here
USERNAME=codabool
...
pacman -Sy archlinux-keyring --noconfirm && sudo pacman -Su --noconfirm >> $BACKUP_LOCATION

# update packages
echo -e "\n========== official packages ==========\n" | tee -a $BACKUP_LOCATION
# performs update on official packages
# then immediately updates AUR if the official update was successful
# updates must be done right after each other to avoid partial upgrades
# https://wiki.archlinux.org/title/system_maintenance#Upgrading_the_system
sudo pacman -Syu --noconfirm \
    | tee -a $BACKUP_LOCATION \
    && echo -e "\n========== AUR packages ==========\n" | tee -a $BACKUP_LOCATION \
    && sudo -u $USERNAME paru -Sua --skipreview --removemake --nocleanafter --pgpfetch --noconfirm \
    | tee -a $BACKUP_LOCATION

echo -e "\n========== flatpak ==========\n" | tee -a $BACKUP_LOCATION
sudo -u $USERNAME /usr/bin/flatpak update --noninteractive --assumeyes | tee -a $BACKUP_LOCATION

echo -e "\n========== update complete, printing new versions ==========\n" >> $BACKUP_LOCATION
write_versions

echo -e "\n========== updated! check for action required in official & AUR section ==========\n" | tee -a $BACKUP_LOCATION
# create a desktop file letting you know a restart should be done
sudo -u $USERNAME touch /home/$USERNAME/Desktop/check_logs_and_restart
```
