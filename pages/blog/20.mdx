...
# 4 Color Spinner
>> This is a live project which can be visited [here](https://spinner-codabool.vercel.app) (it takes about 2 minutes to load)

After seeing many [examples](https://threejs.org/examples/) online and checking out the docs.
I got inspired to createa a Three.js project.
...
If you view [my repository](https://github.com/CodaBool/spinner) you can see that
...
Or so I thought. I noticed that the reflection of the stand still had the outline of the "Wheel of Fortune" so I looked back and noticed the other files involved in the model. There was a metallic roughness and also a normal map which gave the texture data about how to relfect light. I went ahead and did the same process of removing the "Wheel of Fortune" there.

<Img src="/assets/byPost/20/texture.jpg" size="lg" />

# > Write Code

Once that was all done. I had finished the model work and had all the assets to continue to code. I used the helper tool I mentioned at the begining and imported the glb models. From there I altered the scale and rotation to get everything lined up how I would want in the scene. I added a OrbitControls, PerspectiveCamera, and PositionalAudio to the scene. This allowed me to move circle around the object and load in my audio file.
...
> Text rendered in front of the green quater of the spinner
...
Once that is in place I just needed to have an handle an event from the button press with a `onClick={spin}` and it was done. Once again this is a live application and you can try it out yourself [here](https://spinner-codabool.vercel.app). There is no optimization work done so it will talke a few minutes to load. Also if you want to see most of the code is written in the [App.js](https://github.com/CodaBool/spinner/blob/main/src/App.js) file of my repo.
